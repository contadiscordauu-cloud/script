...
-- ==================== SAVE CONFIG SYSTEM ====================
-- Usa writefile/readfile do executor para persistir entre execuções
local SAVE_FILE = "WaveDuels_Config.json"
local saveConfigEnabled = false

-- Estado dos toggles (nome -> bool)
local toggleStates = {
    LEFT        = false,
    RIGHT       = false,
    AUTOBAT     = false,
    ...
    UNWALK      = false,
    FLOAT       = false,
    SAVE_CONFIG = false, -- o próprio toggle save config do settings
    POTATO_MODE = false,
}
...
-- Salva tudo em disco (só age se saveConfigEnabled = true)
...
-- Tenta carregar do disco; retorna a tabela de dados ou nil
...
-- Carrega e aplica dados salvos ANTES de criar UI
local savedData = loadConfigFromDisk()
if savedData then
    -- Só restaura se o save estava habilitado quando foi salvo
    if savedData.saveEnabled then
        saveConfigEnabled = true
        if type(savedData.toggles) == "table" then
            for k, v in pairs(savedData.toggles) do
                if toggleStates[k] ~= nil then
                    toggleStates[k] = v
                end
            end
        end
        if type(savedData.sliders) == "table" then
            for k, v in pairs(savedData.sliders) do
                if sliderValues[k] ~= nil then
                    sliderValues[k] = v
                end
            end
        end
...
-- Declaradas aqui para closeGui poder referenciar
local SettingsFrame, SettingsUIScale, settingsOpened
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        pcall(function()
            if obj:IsA("BasePart") then
                partData[obj] = {Material = obj.Material, Reflectance = obj.Reflectance, CastShadow = obj.CastShadow}
...
SettingsUIScale = Instance.new("UIScale", SettingsFrame)
...
do
    local dr, di, ds, dp = false, nil, nil, nil
    TitleBar.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dr = true; di = input; ds = input.Position; dp = SettingsFrame.Position
        end
    end)
    UserInputService.InputChanged:Connect(function(input)
        if dr and input == di then
            local delta = input.Position - ds
            SettingsFrame.Position = UDim2.new(dp.X.Scale, dp.X.Offset + delta.X, dp.Y.Scale, dp.Y.Offset + delta.Y)
        end
    end)
    UserInputService.InputEnded:Connect(function(input)
        if input == di then dr = false end
    end)
end

-- SCROLL inside settings
...
-- ==================== SLIDER FUNCTION ====================
-- Referências às funções de apply de cada feature (preenchidas depois)
local applyFunctions = {}

local currentlyDraggingSlider = nil

-- Mapeia label do slider -> chave em sliderValues
local sliderKeyMap = {
    ["AUTO SPEED"]  = "autoSpeed",
    ["STEAL SPEED"] = "stealSpeed",
    ["INF JUMP"]    = "infJump",
    ["GRAVITY"]     = "gravity",
    ["SPIN"]        = "spin",
}
...
local function CreateSlider(text, min, max, default)
    -- Usa valor salvo se existir, senão usa default
    local key = sliderKeyMap[text]
    local initialValue = key and sliderValues[key] or default
    ...
    local function setPercent(p)
        ...
        -- Atualiza sliderValues e salva
        if key then
            sliderValues[key] = value
            saveConfig()
        end

        -- Chama apply function se existir (ex: atualizar gravity em tempo real)
        if applyFunctions[text] then
            applyFunctions[text](value)
        end
    end
    ...
    -- Inicializa na posição correta (valor salvo ou default)
    setPercent((initialValue - min) / (max - min))
end
...
local function CreateSettingsToggle(text, callback)
    ...
    -- Determina chave de save para este toggle de settings
    local saveKey = nil
    if text == "SAVE CONFIG" then saveKey = "SAVE_CONFIG"
    elseif text == "POTATO MODE" then saveKey = "POTATO_MODE"
    end

    -- Estado inicial vindo do save
    local state = saveKey and toggleStates[saveKey] or false
    ...
    -- Aplica visual do estado inicial sem chamar callback
    setVisual(state)
    ...
    toggle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            ...
            -- Salva estado deste toggle de settings
            if saveKey then
    ...
    -- Aplica estado inicial via callback (para reativar funcionalidades salvas)
...
-- SAVE CONFIG toggle: liga/desliga o sistema e salva imediatamente ao ligar
CreateSettingsToggle("SAVE CONFIG", function(state)
    saveConfigEnabled = state
    if state then
        -- Salva imediatamente ao ativar
        saveConfig()
    end
end)

-- POTATO MODE toggle
CreateSettingsToggle("POTATO MODE", function(state)
    PotatoEnabled = state
    toggleStates["POTATO_MODE"] = state
    if state then EnablePotato() else DisablePotato() end
    saveConfig()
end)

-- Gear button: UMA única conexão
...
local function adjustGalaxyJump()
    pcall(function()
        local c = player.Character
        if not c then return end
        local humL = c:FindFirstChildOfClass("Humanoid")
        if not humL then return end
        if not galaxyEnabled then
            humL.JumpPower = originalJumpPower
            return
        end
        local percentGravity = (50 - sliderValues.gravity) / 50
        local ratio = math.sqrt((DEFAULT_GRAVITY * percentGravity) / DEFAULT_GRAVITY)
        humL.JumpPower = originalJumpPower * ratio
    end)
end
...
local function doMiniHop()
    ...
    pcall(function()
        ...
        local h    = c:FindFirstChild("HumanoidRootPart")
        local humL = c:FindFirstChildOfClass("Humanoid")
        if not h or not humL then return end
        if tick() - lastHopTime < HOP_COOLDOWN then return end
        lastHopTime = tick()
        if humL.FloorMaterial == Enum.Material.Air then
...
local function captureJumpPower()
    ...
    if c then
        local humL = c:FindFirstChildOfClass("Humanoid")
        if humL and humL.JumpPower > 0 then originalJumpPower = humL.JumpPower end
    end
end
...
local function stopPlayingTracks(humL)
    if not humL then return end
    for _, t in ipairs(humL:GetPlayingAnimationTracks()) do
        pcall(function() t:Stop() end)
    end
end

local function scanAndRemoveAllAnimations(char)
    if not char then return end
    for _, desc in ipairs(char:GetDescendants()) do
        if desc:IsA("Animation") then saveAndRemoveAnimationInstance(desc) end
    end
    local humL = char:FindFirstChildOfClass("Humanoid")
    if humL then stopPlayingTracks(humL) end
end
...
local function createESP(plr)
    ...
    local hrpEsp = char:FindFirstChild("HumanoidRootPart")
    local head   = char:FindFirstChild("Head")
    if not (hrpEsp and head) then return end

    local hitbox = Instance.new("BoxHandleAdornment")
    hitbox.Name       = "ESP_Hitbox"
    hitbox.Adornee    = hrpEsp
    ...
    lbl.TextColor3           = Color3.fromRGB(0, 0, 128)
...
local function antiRemoveRagdollInstant()
    ...
    if humL then
        pcall(function()
            humL.PlatformStand = false
            humL:ChangeState(Enum.HumanoidStateType.GettingUp)
            task.wait()
            humL:ChangeState(Enum.HumanoidStateType.Running)
        end)
    end
    local camera = workspace.CurrentCamera
    if camera and humL and camera.CameraSubject ~= humL then
        pcall(function()
            camera.CameraSubject = humL
...
local function antiStartSystem()
    ...
    antiConnection = RunService.Heartbeat:Connect(function()
        ...
        if not humL or not root then return end
        local state = humL:GetState()
        local vel   = root.AssemblyLinearVelocity
        if humL.FloorMaterial ~= Enum.Material.Air
        ...
        if antiIsBadState(state) then
            ...
            for _, m in char:GetDescendants() do
                if m:IsA("Motor6D") and not m.Enabled then m.Enabled = true end
            end
        end
        ...
        if move.Magnitude > 0 then
            pcall(function() humL:Move(move.Unit, true) end)
        else
            pcall(function() humL:Move(Vector3.zero) end)
        end
        humL.Jump = antiKeysPressed.Space
...
player.CharacterAdded:Connect(function()
    task.wait(0.6)
    if AntiRagdollEnabled then antiStartSystem() end
end)
...
local function wave_getNearestPlayer(hrpL)
    local nearest, dist = nil, math.huge
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            local other = p.Character.HumanoidRootPart
            local d = (Vector3.new(other.Position.X,0,other.Position.Z) - Vector3.new(hrpL.Position.X,0,hrpL.Position.Z)).Magnitude
...
local function wave_stopSpeed()
    if wave_speedConn then pcall(function() wave_speedConn:Disconnect() end) wave_speedConn = nil end
end
...
local function wave_startSpeedTowards(getRootFn, targetPosFn, speed)
    ...
    wave_speedConn = RunService.RenderStepped:Connect(function()
        ...
        if not root or not root.Parent then wave_stopSpeed() return end
        local targetPos = targetPosFn()
        if not targetPos then root.AssemblyLinearVelocity = Vector3.zero return end
...
local function wave_ensureLookSetup(char)
    ...
    local hrpL = char:FindFirstChild("HumanoidRootPart")
    if not hrpL then return end
    local lp = Instance.new("Part")
    lp.Name = "WaveBat_LookPart" lp.Size = Vector3.new(0.2,0.2,0.2)
    lp.Transparency = 1 lp.Anchored = true lp.CanCollide = false lp.Parent = workspace
    local att0 = Instance.new("Attachment", hrpL) att0.Name = "WaveBat_HRP_Att" att0.Position = Vector3.new(0,0,0)
    local att1 = Instance.new("Attachment", lp)   att1.Name = "WaveBat_Target_Att"
    local align = Instance.new("AlignOrientation")
    align.Attachment0 = att0 align.Attachment1 = att1
    align.RigidityEnabled = true align.MaxTorque = 9e8 align.Responsiveness = 200 align.Parent = hrpL
    wave_lookState.lookPart = lp wave_lookState.attHRP = att0 wave_lookState.attTarget = att1 wave_lookState.align = align
end
...
local function wave_restoreWalk(humL)
    if not humL then return end
    if wave_storedWalk[humL] then pcall(function() humL.WalkSpeed = wave_storedWalk[humL] end) wave_storedWalk[humL] = nil end
end

local function wave_fastEquipAndActivateTool(char, humL)
    if not char then return end
    ...
    if not tool then
        ...
        if bp then
            ...
            if tool and humL and typeof(humL.EquipTool) == "function" then
                pcall(function() humL:EquipTool(tool) end) task.wait(0.01)
            end
        end
    end
...
local function WaveBat_Start()
    ...
    wave_hb = RunService.Heartbeat:Connect(function()
        local char = player.Character
        if not char then wave_cleanupLook() wave_stopSpeed() return end
        local hrpL = char:FindFirstChild("HumanoidRootPart")
        local humL = char:FindFirstChildOfClass("Humanoid")
        if not (hrpL and humL) then wave_cleanupLook() wave_stopSpeed() return end
        pcall(function() if humL.Health <= 1 then humL.Health = humL.MaxHealth end end)
        if WaveBat.SafeFollow.ENABLED then
            local targetChar, dist = wave_getNearestPlayer(hrpL)
            if targetChar and targetChar:FindFirstChild("HumanoidRootPart") then
                local trg     = targetChar.HumanoidRootPart
                local flatDir = Vector3.new(trg.Position.X - hrpL.Position.X, 0, trg.Position.Z - hrpL.Position.Z)
                local d       = flatDir.Magnitude
                local desired = WAVE_CLOSE_DISTANCE
                local followPos = trg.Position - (d > 0 and flatDir.Unit * desired or Vector3.new(0,0,0))
                wave_startSpeedTowards(function() return hrpL end, function() return followPos end, WAVE_ACTIVE_WALKSPEED)
                wave_ensureLookSetup(char)
                if wave_lookState.lookPart and wave_lookState.lookPart.Parent then
                    local lookPos = Vector3.new(trg.Position.X, hrpL.Position.Y, trg.Position.Z)
                    wave_lookState.lookPart.CFrame = CFrame.new(hrpL.Position, lookPos)
                end
                if d <= desired + 0.2 then hrpL.AssemblyLinearVelocity = Vector3.zero end
            else
                wave_stopSpeed() wave_restoreWalk(humL) wave_cleanupLook()
            end
        else
            wave_stopSpeed() wave_restoreWalk(humL) wave_cleanupLook()
        end
        if WaveBat.Aimbot.ENABLED then
            ...
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                    local d = (p.Character.HumanoidRootPart.Position - hrpL.Position).Magnitude
                    if d <= dmin then targ = p.Character.HumanoidRootPart dmin = d end
                end
            end
            if targ then
                wave_ensureLookSetup(char)
                if wave_lookState.lookPart and wave_lookState.lookPart.Parent then
                    local lookPos = Vector3.new(targ.Position.X, hrpL.Position.Y, targ.Position.Z)
                    wave_lookState.lookPart.CFrame = CFrame.new(hrpL.Position, lookPos)
                end
                wave_fastEquipAndActivateTool(char, humL)
            end
        end
    end)
end
...
-- Única getClosestPlayer usada por CamFollow e AimAssist
local function getClosestPlayerHRP()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if d < best then best = d closest = p.Character.HumanoidRootPart end
        end
    end
    return closest
end
...
local function cam_ensureLookSetup(char)
    ...
    if not hrpL then return end
    local lp = Instance.new("Part")
    lp.Name = "CamFollow_LookPart" lp.Size = Vector3.new(0.2,0.2,0.2)
    lp.Transparency = 1 lp.Anchored = true lp.CanCollide = false lp.Parent = workspace
    local att0 = Instance.new("Attachment", hrpL) att0.Name = "CamFollow_HRP_Att" att0.Position = Vector3.new(0,0,0)
    local att1 = Instance.new("Attachment", lp)   att1.Name = "CamFollow_Target_Att"
    local align = Instance.new("AlignOrientation")
    align.Attachment0 = att0 align.Attachment1 = att1
    align.RigidityEnabled = true align.MaxTorque = 9e8 align.Responsiveness = 200 align.Parent = hrpL
    cam_lookState.lookPart = lp cam_lookState.attHRP = att0 cam_lookState.attTarget = att1 cam_lookState.align = align
end
...
local function startCamFollow()
    ...
    camFollowConnection = RunService.Heartbeat:Connect(function()
        ...
        local target = getClosestPlayerHRP()
        if not target or not target.Parent then cam_cleanupLook() return end
        ...
        if cam_lookState.lookPart and cam_lookState.lookPart.Parent then
            ...
            pcall(function() cam_lookState.lookPart.CFrame = CFrame.new(root.Position, lookPos) end)
        end
    end)
end
...
local function enableSpin()  spinActive = true end
local function disableSpin()
    spinActive = false
    local r = getHRP()
    if r then pcall(function() r.AssemblyAngularVelocity = Vector3.new(0,0,0) end) end
...
RunService.RenderStepped:Connect(function()
    if spinActive then
        local r = getHRP()
        if r then
            pcall(function() r.AssemblyAngularVelocity = Vector3.new(0, sliderValues.spin * 2, 0) end)
...
local function aim_ensureLookSetup(char)
    ...
    if not hrpL then return end
    local lp = Instance.new("Part")
    lp.Name = "Aim_LookPart" lp.Size = Vector3.new(0.2,0.2,0.2)
    lp.Transparency = 1 lp.Anchored = true lp.CanCollide = false lp.Parent = workspace
    local att0 = Instance.new("Attachment", hrpL) att0.Name = "Aim_HRP_Att" att0.Position = Vector3.new(0,0,0)
    local att1 = Instance.new("Attachment", lp)   att1.Name = "Aim_Target_Att"
    local align = Instance.new("AlignOrientation")
    align.Attachment0 = att0 align.Attachment1 = att1
    align.RigidityEnabled = true align.MaxTorque = 9e8 align.Responsiveness = 200 align.Parent = hrpL
    aim_lookState.lookPart = lp aim_lookState.attHRP = att0 aim_lookState.attTarget = att1 aim_lookState.align = align
end
...
local function getClosestPlayerHead()
    ...
    for _, pl in ipairs(Players:GetPlayers()) do
        if pl ~= player and pl.Character and pl.Character:FindFirstChild("Head") and pl.Character:FindFirstChild("HumanoidRootPart") then
            local d = (myHead.Position - pl.Character.Head.Position).Magnitude
...
local function startAimAssist()
    ...
    aimConnection = RunService.Heartbeat:Connect(function()
        ...
        if not target or not target.Character or not target.Character:FindFirstChild("Head") then
            local humL = player.Character and player.Character:FindFirstChildOfClass("Humanoid")
            if humL then pcall(function() humL.AutoRotate = true end) end
            aim_cleanupLook()
            return
        end
        ...
        if aim_lookState.lookPart and aim_lookState.lookPart.Parent then
            ...
            pcall(function() aim_lookState.lookPart.CFrame = CFrame.new(root.Position, lookAt) end)
        end
        local humL = player.Character and player.Character:FindFirstChildOfClass("Humanoid")
        if humL then pcall(function() humL.AutoRotate = false end) end
    end)
end
...
panelStroke.Color = Color3.fromRGB(40,40,40) panelStroke.Thickness = 1
...
local route1 = {pos1 = Vector3.new(-475.0,-7.0,27.4),  pos2 = Vector3.new(-482.4,-5.1,25.5),  after_pos1 = Vector3.new(-470.0,-7.0,25.1),  after_pos2 = Vector3.new(-472.5,-7.0,100.9)}
local route2 = {pos1 = Vector3.new(-473.20,-7.00,96.09),pos2 = Vector3.new(-485.10,-4.90,95.89),after_pos1 = Vector3.new(-472.96,-7.00,96.31),after_pos2 = Vector3.new(-473.50,-7.00,22.15)}
...
--[[
    createToggle agora:
    1. Lê toggleStates[name] (já populado pelo loadConfig) para saber o estado inicial
    2. Aplica visual imediatamente
    3. Chama a feature correspondente se o estado inicial for true
    4. Em cada clique, salva via saveConfig()
]]
...
local function createToggle(text)
    ...
    -- Estado inicial vindo do save
    local enabled = toggleStates[text] or false
    ...
    local function applyFeature(state)
        ...
        elseif text == "AUTOBAT" then
            ...
            if not state and autoBatRunning then
                WaveBat_Stop()
                autoBatRunning = false
                AutoBatButton.Text             = "Activate"
                AutoBatButton.BackgroundColor3 = Color3.fromRGB(0,122,204)
        ...
        elseif text == "AUTOBACK" then
            autoBackEnabled = state
        elseif text == "XRAY" then
            if tick() - xrayLastToggle < 0.1 then return end
            xrayLastToggle = tick()
            if state then
                xrayGen = xrayGen + 1
                xrayEnabled = true
        ...
        elseif text == "CAMFOLLOW" then
            camFollowEnabled = state
            if state then startCamFollow() else stopCamFollow() end
        elseif text == "FLOAT" then
            if state then enableGravity() else disableGravity() end
        end
    end

    -- Aplica visual inicial (sem animação, instantâneo)
    applyVisual(enabled)

    -- Aplica feature inicial se estava ligado no save
    if enabled then
...
local options = {"LEFT","RIGHT","AUTOBAT","AUTOBACK","XRAY","INFJUMP","SPIN","ANTIRAGDOLL","ESPPLAYER","CAMFOLLOW","UNWALK","FLOAT"}
for _, name in pairs(options) do
    local t = createToggle(name)
    t.Parent = Container
end

-- ==================== DRAG FRAMES PRINCIPAIS ====================
...
local function zeroVelocity()
    local rootLocal = getHRP()
    if rootLocal then pcall(function() rootLocal.AssemblyLinearVelocity = Vector3.zero end) end
end
...
local function walkPath(route)
    ...
    if not autoBackEnabled then stopSpeed() zeroVelocity() running = false activeRoute = nil return end
    task.wait(0.6)
    ...
    stopSpeed() zeroVelocity() running = false activeRoute = nil
end
...
print("✓ WaveDuels carregado com SAVE CONFIG persistente!")
print("✓ Arquivo salvo em: " .. SAVE_FILE)
print("✓ Ative 'SAVE CONFIG' no DUELS CONFIG para persistir entre execuções")
